...
function createLine(): OperationLineDraft {
  return {
...
export function OperationLinesEditor({ products }: { products: InventoryOperationFormOptions["products"] }) {
  const [lines, setLines] = useState<OperationLineDraft[]>(() => [createLine()]);
  const productById = useMemo(() => new Map(products.map((product) => [product.id, product])), [products]);

  function updateLine(id: string, values: Partial<OperationLineDraft>) {
    setLines((current) =>
      current.map((line) => {
        if (line.id !== id) {
          return line;
              ...
              <tr key={line.id} className="border-b border-border/70 align-top">
                ...
                <td className="px-2 py-3">
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    aria-label="Remove operation line"
