...
export default async function PayablesReportPage({ searchParams }: PayablesReportPageProps) {
  ...
  return (
    <PageShell>
      <PageHeader eyebrow="Reports" title="Supplier Payables" />
            ...
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-border">
                <td className="px-4 py-3">
                  <Link href={`/admin/purchasing/vendor-bills/vendor_bill/${row.id}`} className="font-medium text-primary underline-offset-4 hover:underline">
                    {row.billNo}
                  </Link>
